...
async function getNewProduct(req, res) {
    const collections = await prisma_1.default.collection.findMany({ orderBy: { name: 'asc' } });
  ...
  `;
    res.send((0, layout_1.layout)('New Product', body, 'products'));
}
function variantRow(i) {
...
async function postEditProduct(req, res) {
    ...
    try {
        ...
        for (const file of files || []) {
            ...
            if (!error) {
                const { data } = supabase_1.supabaseAdmin.storage
                    .from(process.env.SUPABASE_STORAGE_BUCKET || 'product-images')
                    .getPublicUrl(fileName);
                const currentCount = await prisma_1.default.productImage.count({ where: { productId: id } });
        ...
        const variantEntries = Object.entries(variantsRaw);
...
async function deleteProductImage(req, res) {
    ...
    const image = await prisma_1.default.productImage.findUnique({ where: { id: imageId } });
    if (image) {
